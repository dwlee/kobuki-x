...
void WaiterNode::digitalInputCB(const kobuki_msgs::DigitalInputEvent::ConstPtr& msg)
{
  ...
  if ((msg->values[1] == false) && (delivery_order_.status == simple_delivery_msgs::DeliveryStatus::ERROR))
  {
    ...
    result.message = "Robot manually recovered";
...
//  return;
//  if (order_id == 1)       order_.status = simple_delivery_msgs::DeliveryStatus::ERROR;
//  if (order_id == 2)       order_.status = simple_delivery_msgs::DeliveryStatus::WAITING_FOR_KITCHEN;
//  if (order_id == 3)       order_.status = simple_delivery_msgs::DeliveryStatus::WAITING_FOR_USER_CONFIRMATION;
//  if (order_id == 4)       order_.status = simple_delivery_msgs::DeliveryStatus::IDLE;
